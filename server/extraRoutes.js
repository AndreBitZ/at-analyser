...
export async function extraRoutes(db, req, res, path, method, parts, json, send) {
  ...
  if (method === "GET" && path === "/rosters") {
    send(res, 200, rows(await db.execute(`
      SELECT pts.*, p.name AS player_name, t.name AS team_name, s.label AS season_label
      ...
      ORDER BY s.label DESC, t.name, p.name
    `)));
    return true;
  }
  ...
  if (method === "GET" && path === "/transfers") {
    send(res, 200, rows(await db.execute(`
      SELECT tr.*, p.name AS player_name, a.name AS from_team_name, b.name AS to_team_name, s.label AS season_label
      FROM player_transfers tr
      JOIN players p ON p.id = tr.player_id
      LEFT JOIN teams a ON a.id = tr.from_team_id
  ...
  if (method === "POST" && path === "/transfers") {
    ...
    const tid = id("tr");
    await db.execute({
      sql: "INSERT INTO player_transfers (id, player_id, from_team_id, to_team_id, season_id, notes) VALUES (?, ?, ?, ?, ?, ?)",
      args: [tid, b.player_id, b.from_team_id || null, b.to_team_id, b.season_id, b.notes || null],
    });
    if (b.from_team_id) {
      await db.execute({
        sql: "UPDATE player_team_season SET left_at = datetime('now') WHERE player_id = ? AND team_id = ? AND season_id = ?",
        args: [b.player_id, b.from_team_id, b.season_id],
      });
    }
    await db.execute({
      sql: "INSERT OR IGNORE INTO player_team_season (player_id, team_id, season_id) VALUES (?, ?, ?)",
      args: [b.player_id, b.to_team_id, b.season_id],
    });
    await db.execute({
      sql: "UPDATE player_team_season SET left_at = NULL WHERE player_id = ? AND team_id = ? AND season_id = ?",
      args: [b.player_id, b.to_team_id, b.season_id],
    });
    send(res, 201, { id: tid });
    return true;
