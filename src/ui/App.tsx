...
import {
  formatBox, fieldHeat, goalHeat, matrixZXB, playerScorecard, teamBoxScore, teamShots,
} from "../domain/stats";
...
export default function App() {
  ...
  return <Main root={root} onChangeFolder={() => setRoot(null)} />;
}

function Main({ root, onChangeFolder }: { root: string; onChangeFolder: () => void }) {
  ...
  return (
    <div className="app">
      <header>
        <h1>AT Analyser</h1>
        <p>Pasta: {root} · <button type="button" className="active" onClick={onChangeFolder}>Mudar pasta</button></p>
      </header>
      <div className="tabs">
      ...
      {tab === "clube" && <ClubAdmin />}
      {tab === "jogo" && <div className="grid"><div className="card"><h3>Amostra</h3><div className="stat">{homeBox.goals_scored} — {awayBox.goals_scored}</div></div></div>}
      ...
      {tab === "heat" && (<><select value={teamId} onChange={(e) => setTeamId(e.target.value)}>{teams.map((t) => <option key={t.team_id} value={t.team_id}>{t.name}</option>)}</select><div className="grid" style={{ marginTop: 12 }}><div className="card"><h3>Z1–Z9</h3><div className="heat field">{fHeat.map((c) => <div className="cell" key={c.zone} title={FIELD_ZONE_LABELS[c.zone]}><strong>{c.zone}</strong>{c.volume} · {dash(c.efficiency)}%</div>)}</div></div><div className="card"><h3>B1–B9</h3><div className="heat goal">{gHeat.map((c) => <div className="cell" key={c.zone} title={GOAL_ZONE_LABELS[c.zone]}><strong>{c.zone}</strong>{dash(c.efficiency)}%</div>)}</div></div></div></>)}
...
function Scorecard({ card }: { card: ReturnType<typeof playerScorecard> }) {
  return <div className="card"><h3>{card.player.name}</h3><p>{card.narrative}</p></div>;
}
